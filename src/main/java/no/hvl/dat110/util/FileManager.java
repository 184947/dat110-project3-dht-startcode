...
public class FileManager {
    ...
    public FileManager(NodeInterface chordnode) throws RemoteException {
        ...
        this.numReplicas = Util.numReplicas;
        this.replicafiles = new BigInteger[this.numReplicas];
    }

    public FileManager(NodeInterface chordnode, int N) throws RemoteException {
        this.numReplicas = N;
        this.replicafiles = new BigInteger[N];
        this.chordnode = chordnode;
    }
    ...
    public void createReplicaFiles() {
        ...
        if (replicafiles == null || replicafiles.length != numReplicas) {
            replicafiles = new BigInteger[numReplicas];
        }

        for (int i = 0; i < numReplicas; i++) {
            String replicaName = filename + i;
            BigInteger replica = Hash.hashOf(replicaName);
            replicafiles[i] = replica;
        }
    }
